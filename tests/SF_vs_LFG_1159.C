...
void Analysis::Analyze() {
   ...
   double sigma_SF = 4.5816825e+08 * 1e-38; // cm^2
   double num_events_SF = 4200000;

   double sigma_susa = 1.28967e+09 * 1e-38; // cm^2
   double num_events_susa = 5e6;//19800000;

   double delta_cos = std::abs(std::cos(TMath::DegToRad()*36) - std::cos(TMath::DegToRad()*39));
   double delta_phi = 2*TMath::Pi();
   double solid_angle = delta_cos * delta_phi;  
   ...
   sf_hists = Loop("/pnfs/genie/scratch/users/nsteinbe/eGENIE_grid/e4v/1159/C12/1159_hN_01Q2/EmQ2_01-routine_validation_01-eScattering/e4v_1159_hN_SF.gst.root", sigma_SF, num_events_SF, sf_prot_frac); 
   susa_hists = Loop("/pnfs/genie/persistent/users/apapadop/e4v_SuSav2/Exclusive/electrons/C12_1161GeV/apapadop_SuSav2_C12_1161GeV_master.root", sigma_susa, num_events_susa, susa_prot_frac);
   ...
   TCanvas *c0 = new TCanvas("omega", "omega", 1200, 800);
   sf_hists[0]->SetTitle("SF QE");
   susa_hists[0]->SetTitle("SuSA QE");
   sf_hists[0]->SetLineColor(kRed);
   sf_hists[0]->Scale(1./solid_angle);
   susa_hists[0]->Scale(1./solid_angle);
   sf_hists[0]->Draw("hist same");
   susa_hists[0]->Draw("hist same");
   sf_hists[0]->GetXaxis()->SetTitle("Energy Transfer (GeV)");
   sf_hists[0]->GetYaxis()->SetTitle("#frac{d#sigma}{dEd#Omega}} (#mu b/GeV/sr)");
   c0->BuildLegend();

   TCanvas *c1 = new TCanvas("emiss", "emiss", 1200, 800);
   sf_hists[1]->SetTitle("SF");
   sf_hists[1]->GetXaxis()->SetTitle("Missing Energy (GeV)");
   sf_hists[1]->GetYaxis()->SetTitle("#frac{d#sigma}{dE_{m}} (#mu b/GeV)");
   sf_hists[1]->SetLineColor(kRed);
   susa_hists[1]->SetTitle("LFG SuSA");
   susa_hists[1]->SetLineColor(kBlue);
   sf_hists[1]->Draw("hist same");
   susa_hists[1]->Draw("hist same");
   c1->BuildLegend();

   TCanvas *c2 = new TCanvas("pmiss", "pmiss", 1200, 800);
   susa_hists[2]->SetTitle("SuSAv2");
   sf_hists[2]->SetTitle("SF");
   sf_hists[2]->GetXaxis()->SetTitle("Missing Momentum (GeV)");
   sf_hists[2]->GetYaxis()->SetTitle("#frac{d#sigma}{dP_{m}} (#mu b/GeV)");
   sf_hists[2]->SetLineColor(kRed); sf_hists[3]->SetLineColor(kRed);
   sf_hists[4]->SetLineStyle(kDashed);
   susa_hists[2]->SetLineColor(kBlue); susa_hists[3]->SetLineColor(kBlue);
   susa_hists[4]->SetLineStyle(kDashed);
   sf_hists[2]->Draw("HIST SAME");
   sf_hists[4]->Draw("HIST SAME");
   susa_hists[2]->Draw("HIST SAME");
   susa_hists[4]->Draw("HIST SAME");
   c2->BuildLegend();

   TCanvas *c3 = new TCanvas("nuc mom", "nuc mom", 1200, 800);
   sf_hists[5]->SetTitle("SF");
   sf_hists[5]->GetXaxis()->SetTitle("P_{n} (Struck nucleon momentum) (GeV)");
   sf_hists[5]->GetYaxis()->SetTitle("#frac{d#sigma}{dP_{n}} (#mu b/GeV)");
   sf_hists[5]->SetLineColor(kRed);
   susa_hists[5]->SetTitle("LFG SuSA");
   susa_hists[5]->SetLineColor(kBlue);
   sf_hists[5]->Draw("hist same");
   susa_hists[5]->Draw("hist same");
   c3->BuildLegend();

   TCanvas *c4 = new TCanvas("nuc mom final", "nuc mom final", 1200, 800);
   sf_hists[6]->SetTitle("SF");
   susa_hists[6]->SetTitle("SuSAv2 LFQ");
   sf_hists[6]->GetXaxis()->SetTitle("P_{p} (Proton momentum) (GeV)");
   sf_hists[6]->GetYaxis()->SetTitle("#frac{d#sigma}{dP_{p}} (#mu b/GeV)");
   sf_hists[6]->SetLineColor(kRed); sf_hists[7]->SetLineColor(kRed);
   sf_hists[8]->SetLineStyle(kDashed);
   susa_hists[6]->SetLineColor(kBlue); susa_hists[7]->SetLineColor(kBlue);
   susa_hists[8]->SetLineStyle(kDashed);
   sf_hists[6]->Draw("HIST SAME");
   sf_hists[8]->Draw("HIST SAME");
   susa_hists[6]->Draw("HIST SAME");
   susa_hists[8]->Draw("HIST SAME");
   c4->BuildLegend();
...
std::vector<TH1D*> Analysis::Loop(TString filename, double tot_xsec, double num_events, double &prot_fraction)
{
   ...
   TH1D* h_omega = new TH1D("omega", "omega", 20, 0, 0.8);
   ...
   TH1D* h_nuc_final_thetaPQ = new TH1D("P_{p} (#theta_{PQ} < 20.^{#circ})", "P_{m} (#theta_{PQ} < 20.^{#circ})", 50, 0.3, 1.2);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      double theta_e_deg = std::acos(cthl)*TMath::RadToDeg();
      if(pass_elec_cut(pl, theta_e_deg) == false ) continue;

      //if(theta_e_deg < 28.0 || theta_e_deg > 31.0) continue;


      if(theta_e_deg < 36.0 || theta_e_deg > 39.0) continue;
      ...
      qvec.SetXYZ(-pxl, -pyl, 1.161 - pzl);

      double omega = 1.161 - El;
