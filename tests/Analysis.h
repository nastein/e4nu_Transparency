...
// Thu Jun  9 14:40:04 2022 by ROOT version 6.25/01
// from TTree gst/GENIE Summary Event Tree
// found on file: eC12_1000.gst.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   // Declaration of leaf types, these are all of the variables stored in the TTree!
   Int_t           iev;
   Int_t           neu;
   Int_t           fspl;
   Int_t           tgt;
   ...
   //std::vector<TH1D*>     Loop(TString filename, double tot_xsec, double num_events, double& prot_fraction);
   void Loop(TString filename, int range);
   void bin_normalize(TH1D *h);
   virtual Bool_t   Notify();
   virtual void     Show(Long64_t entry = -1);
   virtual std::string number_to_string(double num);
};
...
std::string Analysis::number_to_string(double num) {
  std::stringstream ss;
  ss << num;
  return ss.str();
}
