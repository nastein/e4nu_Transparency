...
// --------------------- Getting MC exclusive and inclusive measurements and mec/neutron corrections ----------------
        ...
        for(int p = 0; p < 2; p++) {
                for(int r = 0; r < 2; r++) {
                        ...
                        UniversalE4vFunction(SuSA_1161[r][p], FSIModelsToLabels["SuSav2_NoRadCorr_LFGM_Truth_WithFidAcc"], "12C","1_161", TString::Format("susa_1161_mom_reco_%i%i",r,p));
                }
        }
        ...
        for(int p = 0; p < 2; p++) {
                for(int r = 0; r < 3; r++) {
                        ...
                        UniversalE4vFunction(SuSA_2261[r][p], FSIModelsToLabels["SuSav2_NoRadCorr_LFGM_Truth_WithFidAcc"], "12C","2_261", TString::Format("susa_2261_mom_reco_%i%i",r,p));
                }
        }
        ...
        for(int p = 0; p < 2; p++) {
                for(int r = 0; r < 1; r++) {
                        ...
                        UniversalE4vFunction(SuSA_4461[r][p], FSIModelsToLabels["SuSav2_NoRadCorr_LFGM_Truth_WithFidAcc"], "12C","4_461", TString::Format("susa_4461_mom_reco_%i%i",r,p));
                }
        }
...
// --------------------- Getting Data exclusive and inclusive measurements ----------------
        ...
        for(int p = 0; p < 2; p++) {
                ...
                UniversalE4vFunction(Data_1161[0][p], "Pinned Data", "12C", "1_161", TString::Format("data_1161_mom_0%i",p));
                UniversalE4vFunction(Data_1161[1][p], "Pinned Data", "12C", "1_161", TString::Format("data_1161_mom_1%i",p));
                UniversalE4vFunction(Data_2261[0][p], "Pinned Data", "12C", "2_261", TString::Format("data_2261_mom_0%i",p));
                UniversalE4vFunction(Data_2261[1][p], "Pinned Data", "12C", "2_261", TString::Format("data_2261_mom_1%i",p));
                UniversalE4vFunction(Data_2261[2][p], "Pinned Data", "12C", "2_261", TString::Format("data_2261_mom_2%i",p));
                UniversalE4vFunction(Data_4461[0][p], "Pinned Data", "12C", "4_461", TString::Format("data_4461_mom_0%i",p));
        }
	...
	if(make_plots) {
        ...
        for(int p = 0; p < 2; p++) {
		        ...
		        SuSA_1161[r][p]->SetTitle(TString::Format("SuSA1161_%s",first_en[r][p].c_str()));
                        ...
                        SuSA_1161[r][p] = (TH1D*)AcceptanceCorrect(SuSA_1161[r][p], SuSA_1161_true[r][p], SuSA_1161_true_reco[r][p], G_1161_true[r][p], G_1161_true_reco[r][p], SuSA_1161_true_reco_rad[r][p], make_plots, false);
                }
        }
...
// ---------------- Make plots of reconstructed momentum distributions for acc corrected data/MC ------------
        if(make_plots) {
                ...
                for(int p = 0; p < 2; p++) {
                        for(int r = 0; r< 2; r++ ) {
                                SuSA_1161[r][p]->Draw("e hist");
                                Data_1161[r][p]->Draw("e same");
                                SuSA_1161[r][p]->SetTitle("SuSA_v2 C12");
                                Data_1161[r][p]->SetTitle("Data C12");
                                Data_1161[r][p]->SetMarkerColor(kBlack);
                                Data_1161[r][p]->SetMarkerStyle(20);
                                SuSA_1161[r][p]->GetYaxis()->SetTitle("Absolute Number of Events ");
                                SuSA_1161[r][p]->GetXaxis()->SetTitle("Momentum (GeV)");
                                reco_dist_canvas->BuildLegend();
                                SuSA_1161[r][p]->SetTitle(TString::Format("SuSA_v2 C12 %s", first_en[r][p].c_str()));
                                reco_dist_canvas->SaveAs("C12_reco_dist.pdf");
                        }
                }
                ...
                for(int p = 0; p < 2; p++) {
                        for(int r = 0; r< 3; r++ ) {
                                ...
                                SuSA_2261[r][p]->SetTitle("SuSA_v2 C12");
                                Data_2261[r][p]->SetTitle("Data C12");
                                ...
                                reco_dist_canvas->BuildLegend();
                                SuSA_2261[r][p]->SetTitle(TString::Format("SuSA_v2 C12 %s", second_en[r][p].c_str()));
                                reco_dist_canvas->SaveAs("C12_reco_dist.pdf");
                        }
                }
                ...
                for(int p = 0; p < 2; p++) {
                        for(int r = 0; r< 1; r++ ) {
                                ...
                                SuSA_4461[r][p]->SetTitle("SuSA_v2 C12");
                                Data_4461[r][p]->SetTitle("Data C12");
                                ...
                                reco_dist_canvas->BuildLegend();
                                SuSA_4461[r][p]->SetTitle(TString::Format("SuSA_v2 C12 %s", third_en[r][p].c_str()));
                                reco_dist_canvas->SaveAs("C12_reco_dist.pdf");
                        }
                }
                reco_dist_canvas->SaveAs("C12_reco_dist.pdf)");
