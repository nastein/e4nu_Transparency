...
void genie_analysis::Loop(Int_t choice) {
	...
	bool ApplyThetaSlice = false; double MinThetaSlice = 27, MaxThetaSlice = 30;
...
//	TH1F *h1_EePrime_FullyInclusive_NoQ4Weight_SecondSector_Theta_Slice_xBCut = new TH1F("h1_EePrime_FullyInclusive_NoQ4Weight_SecondSector_Theta_Slice_xBCut","",6000,0.,6.);
	...
	int NBinsOscStudies = 100; double MinOscStudies = 0., MaxOscStudies = 10.;

	TH1F *h1_EQE_SuperFine=new TH1F("h1_EQE_SuperFine","",NBinsOscStudies*NBinsOscStudies,MinOscStudies,MaxOscStudies);
	TH1F *h1_Ecal_SuperFine=new TH1F("h1_Ecal_SuperFine","",NBinsOscStudies*NBinsOscStudies,MinOscStudies,MaxOscStudies);

	TH1F *h1_EQE=new TH1F("h1_EQE","",NBinsOscStudies,MinOscStudies,MaxOscStudies);
	TH1F *h1_Ecal=new TH1F("h1_Ecal","",NBinsOscStudies,MinOscStudies,MaxOscStudies);
	TH2F *h2_Ecal_Etrue=new TH2F("h2_Ecal_Etrue","",NBinsOscStudies,MinOscStudies,MaxOscStudies,NBinsOscStudies,MinOscStudies,MaxOscStudies);
	TH2F *h2_Etrue_Ecal=new TH2F("h2_Etrue_Ecal","",NBinsOscStudies,MinOscStudies,MaxOscStudies,NBinsOscStudies,MinOscStudies,MaxOscStudies);
...
//	TH2F *h2_el_theta_phi = new TH2F("h2_el_theta_phi","",200,0,360,200,0,180);
	...
	TH2F *h2_PiMinus_Theta_Momentum_ThirdSector = new TH2F("h2_PiMinus_Theta_Momentum_ThirdSector",";P_{#pi^{-}} [GeV/c];#theta_{#pi^{-}}",6000,0.,6,360,0,360);
	TH2F *h2_PiMinus_Theta_Momentum_FourthSector = new TH2F("h2_PiMinus_Theta_Momentum_FourthSector",";P_{#pi^{-}} [GeV/c];#theta_{#pi^{-}}",6000,0.,6,360,0,360);
	TH2F *h2_PiMinus_Theta_Momentum_FifthSector = new TH2F("h2_PiMinus_Theta_Momentum_FifthSector",";P_{#pi^{-}} [GeV/c];#theta_{#pi^{-}}",6000,0.,6,360,0,360);
	TH2F *h2_PiMinus_Theta_Momentum_SixthSector = new TH2F("h2_PiMinus_Theta_Momentum_SixthSector",";P_{#pi^{-}} [GeV/c];#theta_{#pi^{-}}",6000,0.,6,360,0,360);
	...
	int NBinsW = 2000; double MinW = 0., MaxW = 3.;
	...
	TH1F *h1_ECal_InThetaSlices[ThetaSlices];
	...
	TH1F *h1_ECal_InEePrimeSlices[EePrimeSlices];
...
//	double MinCosTheta2D = 0.55, MaxCosTheta2D = 1.; int CosThetaSlices2D = 3;	
	...
	if(en_beam[fbeam_en]>1. && en_beam[fbeam_en]<2.) { MaxCosTheta2D = 0.96; }
	if(en_beam[fbeam_en]>2. && en_beam[fbeam_en]<3.) { MaxCosTheta2D = 0.96; }
...
//		
//			h1_ECal_InEePrimeAndThetaSlices[WhichEePrimeSlice2D][WhichThetaSlice2D] = new TH1F(Form("h1_ECal_InEePrime_%d_To_%d_InTheta_%d_To_%d_Slices",MinEePrimeSlice2D,MaxEePrimeSlice2D,MinThetaSlice2D,MaxThetaSlice2D),"",n_bins,x_values);
//			
//		}

		for (int WhichCosThetaSlice2D = 0 ; WhichCosThetaSlice2D < CosThetaSlices2D; WhichCosThetaSlice2D++ ) {	
		
			//int MinCosThetaSlice2D = MinCosTheta2D+WhichCosThetaSlice2D*CosThetaStep2D;
			//int MaxCosThetaSlice2D = MinCosTheta2D+(WhichCosThetaSlice2D+1)*CosThetaStep2D;	
...
//			h1_ECal_InEePrimeAndThetaSlices[WhichEePrimeSlice2D][WhichCosThetaSlice2D] = new TH1F(Form("h1_ECal_InEePrime_%d_To_%d_InCosTheta_%d_To_%d_Slices",MinEePrimeSlice2D,MaxEePrimeSlice2D,MinCosThetaSlice2D,MaxCosThetaSlice2D),"",n_bins,x_values);
	...
	// Dolan's suggestion: EQE_0pi reso for T2K

	int T2KNBins = 200; double T2KMinEQEReso = -1, T2KMaxEQEReso = 1;

	int T2KEePrimeSlices2D = 3;
	double T2KMinEePrime2D = -1, T2KMaxEePrime2D = -1;

	if(en_beam[fbeam_en]>1. && en_beam[fbeam_en]<2.) { T2KMinEePrime2D = 0.5; T2KMaxEePrime2D = 1.1; }
	if(en_beam[fbeam_en]>2. && en_beam[fbeam_en]<3.) { T2KMinEePrime2D = 1.3; T2KMaxEePrime2D = 2.2; }
	if(en_beam[fbeam_en]>4. && en_beam[fbeam_en]<5.) { T2KMinEePrime2D = 2.5; T2KMaxEePrime2D = 4.; }

	double T2KEePrimeStep2D = (T2KMaxEePrime2D - T2KMinEePrime2D) / T2KEePrimeSlices2D;	
	double T2KMinCosTheta2D = 0.6, T2KMaxCosTheta2D = 1.; int T2KCosThetaSlices2D = 3;	

	if(en_beam[fbeam_en]>1. && en_beam[fbeam_en]<2.) { T2KMaxCosTheta2D = 0.96; }
	if(en_beam[fbeam_en]>2. && en_beam[fbeam_en]<3.) { T2KMaxCosTheta2D = 0.96; }

	double T2KCosThetaStep2D = (T2KMaxCosTheta2D - T2KMinCosTheta2D) / T2KCosThetaSlices2D;
	TH1F *h1_T2KEQEReso_InEePrimeAndCosThetaSlices[NInt][T2KEePrimeSlices2D][T2KCosThetaSlices2D];
	TH2D *h2_T2K_Q0VsQ3_InEePrimeAndCosThetaSlices[NInt][T2KEePrimeSlices2D][T2KCosThetaSlices2D];

	for (int T2KWhichEePrimeSlice2D = 0 ; T2KWhichEePrimeSlice2D < T2KEePrimeSlices2D; T2KWhichEePrimeSlice2D++ ) {

		for (int T2KWhichCosThetaSlice2D = 0 ; T2KWhichCosThetaSlice2D < T2KCosThetaSlices2D; T2KWhichCosThetaSlice2D++ ) {	

			for (int WhichInt = 0; WhichInt < NInt; WhichInt++) {

				h1_T2KEQEReso_InEePrimeAndCosThetaSlices[WhichInt][T2KWhichEePrimeSlice2D][T2KWhichCosThetaSlice2D] = new TH1F(Form("h1_T2KEQEReso_Int_%d_InEePrime_%d_To_%d_InCosTheta_%d_To_%d_Slices",WhichInt,T2KWhichEePrimeSlice2D,T2KWhichEePrimeSlice2D+1,T2KWhichCosThetaSlice2D,T2KWhichCosThetaSlice2D+1),";(e,e')_{0#pi} E_{QE} Reso",T2KNBins,T2KMinEQEReso,T2KMaxEQEReso);

				h2_T2K_Q0VsQ3_InEePrimeAndCosThetaSlices[WhichInt][T2KWhichEePrimeSlice2D][T2KWhichCosThetaSlice2D] = new TH2D(Form("h1_T2K_Q0VsQ3_Int_%d_InEePrime_%d_To_%d_InCosTheta_%d_To_%d_Slices",WhichInt,T2KWhichEePrimeSlice2D,T2KWhichEePrimeSlice2D+1,T2KWhichCosThetaSlice2D,T2KWhichCosThetaSlice2D+1),";(e,e')_{0#pi} q_{0} [GeV];(e,e')_{0#pi} q_{3} [GeV]",400,0,4,400,0,6);
...
//		rotation->PrintQVector();
		...
		int T2KCosThetaSlice2D = (V3_el.CosTheta() - T2KMinCosTheta2D) / T2KCosThetaStep2D;
		int T2KEePrimeSlice2D = (V4_el.E() - T2KMinEePrime2D) / T2KEePrimeStep2D;
		...
		if (num_p == 2) {
			...
			if (num_pi_phot == 0 && N_prot_both != 0){
				...
				for (int f = 0; f < num_p; f++) { // Looping through two protons
					...
					h1_EQE->Fill(E_rec,-P_N_2p[f]*histoweight);
					...
					if (ThetaSlice > -1 && ThetaSlice < ThetaSlices && EePrimeSlice > -1 && EePrimeSlice < EePrimeSlices && Q2Slice > -1 && Q2Slice < Q2Slices) {
						...
						h1_ECal_InThetaSlices[ThetaSlice]->Fill(E_tot_2p[f],LocalWeight);
						h1_ECal_InEePrimeSlices[EePrimeSlice]->Fill(E_tot_2p[f],LocalWeight);
//						h1_ECal_InEePrimeAndThetaSlices[EePrimeSlice2D][ThetaSlice2D]->Fill(E_tot_2p[f],LocalWeight);
...
//						pion_acc_ratio = acceptance_c(pion_mom_corr, cos(pion_theta), phi_pion, -211, file_acceptance,false);
				...
				for (int z=0; z < N_2prot; z++){ //looping over two protons
					...
					h1_EQE->Fill(E_rec,P_2p1pito2p0pi[z]*histoweight);
					...
					if (ThetaSlice > -1 && ThetaSlice < ThetaSlices && EePrimeSlice > -1 && EePrimeSlice < EePrimeSlices && Q2Slice > -1 && Q2Slice < Q2Slices ) {
						...
						h1_ECal_InEePrimeSlices[EePrimeSlice]->Fill(Ecal_2p1pi_to2p0pi[z],LocalWeight);
//						h1_ECal_InEePrimeAndThetaSlices[EePrimeSlice2D][ThetaSlice2D]->Fill(Ecal_2p1pi_to2p0pi[z],LocalWeight);
//						h1_ECal_InEePrimeAndCosThetaSlices[EePrimeSlice2D][CosThetaSlice2D]->Fill(Ecal_2p1pi_to2p0pi[z],LocalWeight);
					...
					h1_EQE->Fill(E_rec,P_2p1pito1p1pi[z]*histoweight);
					...
					if (ThetaSlice > -1 && ThetaSlice < ThetaSlices && EePrimeSlice > -1 && EePrimeSlice < EePrimeSlices && Q2Slice > -1 && Q2Slice < Q2Slices) {
						...
						h1_ECal_InThetaSlices[ThetaSlice]->Fill(E_tot_2p[z],LocalWeight);
						h1_ECal_InEePrimeSlices[EePrimeSlice]->Fill(E_tot_2p[z],LocalWeight);
//						h1_ECal_InEePrimeAndThetaSlices[EePrimeSlice2D][ThetaSlice2D]->Fill(E_tot_2p[z],LocalWeight);			
//						h1_ECal_InEePrimeAndCosThetaSlices[EePrimeSlice2D][CosThetaSlice2D]->Fill(E_tot_2p[z],LocalWeight);			
					...
					h1_EQE->Fill(E_rec,-P_2p1pito1p0pi[z]*histoweight);
...
//							pion_acc_ratio[i] = acceptance_c(pion_mom_corr, cos(pion_theta), phi_pion, -211, file_acceptance,false);
				...
				for(int z = 0; z < N_2prot; z++){ //looping over two protons
					...
					h1_EQE->Fill(E_rec,Ptot_2p[z]*histoweight);
			...
			if (num_pi_phot == 0 && N_p_three != 0){
				...
				for(int count = 0; count < N_comb; count++) { // Loop over number of combinations
					...
					for(int j = 0; j < N_2p; j++) { // Loop over two protons
						...
						h1_EQE->Fill(E_rec,P_3pto2p[count][j]*histoweight);
						...
						if (ThetaSlice > -1 && ThetaSlice < ThetaSlices && EePrimeSlice > -1 && EePrimeSlice < EePrimeSlices && Q2Slice > -1 && Q2Slice < Q2Slices) {
							...
							h1_ECal_InThetaSlices[ThetaSlice]->Fill(E_cal_3pto2p[count][j],LocalWeight);
							h1_ECal_InEePrimeSlices[EePrimeSlice]->Fill(E_cal_3pto2p[count][j],LocalWeight);
//							h1_ECal_InEePrimeAndThetaSlices[EePrimeSlice2D][ThetaSlice2D]->Fill(E_cal_3pto2p[count][j],LocalWeight);
//							h1_ECal_InEePrimeAndCosThetaSlices[EePrimeSlice2D][CosThetaSlice2D]->Fill(E_cal_3pto2p[count][j],LocalWeight);
				...
				for(int j = 0; j < N_3p; j++)    {
					...
					h1_EQE->Fill(E_rec,-P_3pto1p[j]*histoweight);
					...
					if (ThetaSlice > -1 && ThetaSlice < ThetaSlices && EePrimeSlice > -1 && EePrimeSlice < EePrimeSlices && Q2Slice > -1 && Q2Slice < Q2Slices ) {
						...
						h1_ECal_InThetaSlices[ThetaSlice]->Fill(E_cal[j],LocalWeight);
						h1_ECal_InEePrimeSlices[EePrimeSlice]->Fill(E_cal[j],LocalWeight);
//						h1_ECal_InEePrimeAndThetaSlices[EePrimeSlice2D][ThetaSlice2D]->Fill(E_cal[j],LocalWeight);
...
//						pion_acc_ratio = acceptance_c(pion_mom_corr, cos(pion_theta), phi_pion, -211, file_acceptance,false);
				...
				for(int j = 0; j < N_3p; j++) { //loop over 3 protons
					...
					h1_EQE->Fill(E_rec,P_tot_3p[j]*histoweight);
...
*/
		...
		if (num_pi_phot == 0) {
			...
			if (T2KCosThetaSlice2D > -1 && T2KCosThetaSlice2D < T2KCosThetaSlices2D && T2KEePrimeSlice2D > -1 && T2KEePrimeSlice2D < T2KEePrimeSlices2D) {
		...
		if( num_p == 1) {
			...
			if(num_pi_phot == 0){
				...
				if (ThetaSlice > -1 && ThetaSlice < ThetaSlices && EePrimeSlice > -1 && EePrimeSlice < EePrimeSlices && Q2Slice > -1 && Q2Slice < Q2Slices) {
					...
					h1_ECal_InThetaSlices[ThetaSlice]->Fill(E_tot,LocalWeight);
					h1_ECal_InEePrimeSlices[EePrimeSlice]->Fill(E_tot,LocalWeight);
//					h1_ECal_InEePrimeAndThetaSlices[EePrimeSlice2D][ThetaSlice2D]->Fill(E_tot,LocalWeight);				
...
//						pion_acc_ratio = acceptance_c(pion_mom_corr, cos(pion_theta), phi_pion, -211, file_acceptance,false);
				...
				if (N_piphot_det != 0) {
					...
					h1_EQE->Fill(E_rec,-(N_piphot_undet/N_piphot_det)*histoweight);
...
//							pion_acc_ratio[i] = acceptance_c(pion_mom_corr, cos(pion_theta), phi_pion, -211, file_acceptance,false);
				...
				for(int z = 0; z < N_2pi; z++){  //to consider 2 diff. 1pi states
					...
					h1_EQE->Fill(E_rec,P_1p1pi[z]*histoweight);
...
//						h1_ECal_InEePrimeAndCosThetaSlices[EePrimeSlice2D][CosThetaSlice2D]->Fill(E_tot,LocalWeight);
				...
				h1_EQE->Fill(E_rec,-P_1p0pi*histoweight);
...
//							pion_acc_ratio[i] = acceptance_c(pion_mom_corr, cos(pion_theta), phi_pion, -211, file_acceptance,false);
				...
				h1_EQE->Fill(E_rec,P_1p3pi*histoweight);
...
//	TH1F *h_Etot_subtruct_piplpimi_factor =(TH1F*)  h1_E_tot_cut2->Clone("epRecoEnergy_slice_0");
	...
	h_Erec_subtruct_piplpimi_factor_fracfeed->Add(h1_E_rec_undetfactor_fracfeed,-1); // 1pi subtraction
