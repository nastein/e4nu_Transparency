...
// --------------------- Getting acceptance correction histograms from MC ---------------
        ...
        //[NRanges][Exclusive = 0, Inclusive = 1]     
...
// --------------------- Getting MC exclusive and inclusive measurements and mec/neutron corrections ----------------
        ...
        for(int p = 0; p < 2; p++) {
                for(int r = 0; r < 3; r++) {
                        ...
                        UniversalE4vFunction(SuSA_2261[r][p], FSIModelsToLabels["SuSav2_NoRadCorr_LFGM_Truth_WithFidAcc"], "4He","2_261", TString::Format("susa_2261_mom_reco_%i%i",r,p));
                }
        }
        ...
        for(int p = 0; p < 2; p++) {
                for(int r = 0; r < 1; r++) {
                        ...
                        UniversalE4vFunction(SuSA_4461[r][p], FSIModelsToLabels["SuSav2_NoRadCorr_LFGM_Truth_WithFidAcc"], "4He","4_461", TString::Format("susa_4461_mom_reco_%i%i",r,p));
                }
        }
...
// --------------------- Getting Data exclusive and inclusive measurements ----------------
        ...
        for(int p = 0; p < 2; p++) {
                ...
                UniversalE4vFunction(Data_2261[0][p], "Pinned Data", "4He", "2_261", TString::Format("data_2261_mom_0%i",p));
                UniversalE4vFunction(Data_2261[1][p], "Pinned Data", "4He", "2_261", TString::Format("data_2261_mom_1%i",p));
                UniversalE4vFunction(Data_2261[2][p], "Pinned Data", "4He", "2_261", TString::Format("data_2261_mom_2%i",p));
                UniversalE4vFunction(Data_4461[0][p], "Pinned Data", "4He", "4_461", TString::Format("data_4461_mom_0%i",p));
        }
...
// ---------------------------------------------------------------------
        ...
        SuSA_t->SetTitle("SuSA_v2 He4");
        SuSA_t->Draw("P");
        
        c->BuildLegend();
        c->SaveAs("T_He4.pdf");

        TFile *final = TFile::Open("T_He4.root", "RECREATE");
        data_t->Write("data");
        SuSA_t->Write("SuSA");
        final->Close();
}
