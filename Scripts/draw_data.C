...
std::vector<TGraphErrors*> fillAdep(std::vector<TGraphErrors*> He, std::vector<TGraphErrors*> C, std::vector<TGraphErrors*> Fe) {
        ...
        for(int range = 0; range < 3; range++) {
                ...
                legA[range]->AddEntry(d[range], TString::Format("#splitline{Data: #alpha = %3.2f}{+/- %3.2f}", r_data[range]->Parameter(0),r_data[range]->ParError(0)));
                legA[range]->AddEntry(s[range], TString::Format("#splitline{SuSAv2: #alpha = %3.2f}{+/- %3.2f}", r_susa[range]->Parameter(0),r_susa[range]->ParError(0)));
                legA[range]->AddEntry(g[range], TString::Format("#splitline{G18: #alpha = %3.2f}{+/- %3.2f}", r_g18[range]->Parameter(0),r_g18[range]->ParError(0)));
