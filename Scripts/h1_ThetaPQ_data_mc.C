...
void h1_ThetaPQ_data_mc( std::string sim, std::string targ, std::string beamen, std::string range, std::string type ){
	...
	for (int i = 0; i < 4; i++) { // for all the interactions
	    for (int j = 0; j < 6; j++) { // for all the sectors
            ...
            if(i == 0 && j == 0) h1_ThetaPQ_mc_total = (TH1D*)h1_ThetaPQ_InteractionBreakDown_inSector_mc[i][j]->Clone();
            if(i != 0 ||  j != 0) h1_ThetaPQ_mc_total->Add(h1_ThetaPQ_InteractionBreakDown_inSector_mc[i][j]);
          if(j == 0) h1_ThetaPQ_mc_int[i] = (TH1D*)h1_ThetaPQ_InteractionBreakDown_inSector_mc[i][j]->Clone();
        if(j != 0) h1_ThetaPQ_mc_int[i]->Add(h1_ThetaPQ_InteractionBreakDown_inSector_mc[i][j]);
   ...
   UniversalE4vFunction(h1_ThetaPQ_data[0],  "Pinned Data", target, beam_en, TString(info2));
    ...
    h1_ThetaPQ_data[0]->GetXaxis()->SetTitle("ThetaPQ (Degrees)");
    h1_ThetaPQ_data[0]->GetYaxis()->SetTitle("Events");
    h1_ThetaPQ_data[0]->GetXaxis()->CenterTitle( true);
    h1_ThetaPQ_data[0]->GetYaxis()->CenterTitle( true);
    h1_ThetaPQ_data[0]->SetTitle(TString(info2)+TString(cuts2));
    h1_ThetaPQ_data[0]->SetMarkerStyle(2);
    h1_ThetaPQ_data[0]->SetMarkerColor(kBlack);
    
    h1_ThetaPQ_mc_total->SetLineColor(kRed);
    h1_ThetaPQ_mc_total->GetYaxis()->SetRangeUser(0, 1.1*h1_ThetaPQ_mc_total->GetMaximum());
    h1_ThetaPQ_mc_total->Draw("HIST");
    for(int i = 0; i < 4; i++) {
      h1_ThetaPQ_mc_int[i]->SetLineColor(color_options[i]);
      h1_ThetaPQ_mc_int[i]->Draw("HIST SAME");
    }
    h1_ThetaPQ_data[0]->Draw("HIST E SAME");
