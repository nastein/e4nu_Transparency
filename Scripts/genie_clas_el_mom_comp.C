...
double genie_clas_el_mom_comp_pt_converter( std::string str) {
...
// -----------------------------------------------------------------------------------------------------------------------
// This auxillary function utilizes genie_clas_el_mom_comp_pt_converter to take the resulting
// value from genie_clas_el_mom_comp_pt_converter, remove any trailing zeros, and then recast
// the double from genie_clas_el_mom_comp_pt_converter to be a std::string.
std::string genie_clas_el_mom_comp_pt_plot_converter( std::string str) {
    str = std::to_string( genie_clas_el_mom_comp_pt_converter( str)); // replace 'pt' -> '.' and recast as a std::string
...
// MAIN FUNCTION ---------------------------------------------------------------------------------------------------------
void genie_clas_el_mom_comp( std::string mc_file_name, std::string data_file_name){
    // This COMMONLY EDITED SECTION contains variables that a user may want to routinely modify.
    Int_t rebin_factor = 10; // how many bins to merge together into one for the final histogram; h1_el_mom plots created by genie_analysis range from 0 to 6 and have 6000 bins
    ...
    else { std::cout << "You have entered an invalid/unknown target. Consider adding code to accomodate this target in genie_clas_el_mom_comp.C." << std::endl; }
    ...
    else { std::cout << "You have entered an invalid/unknown beam energy. Consider adding code to accomodate this beam energy in genie_clas_el_mom_comp.C" << std::endl; }
    ...
    if (deltaPhiElEq_kwd_pos != std::string::npos){
        ...
        deltaPhiEl_plot = genie_clas_el_mom_comp_pt_plot_converter( deltaPhiEl);
    }
    ...
    if (thetaEl_kwd_pos != std::string::npos){
        ...
        thetaEl_lb_plot = genie_clas_el_mom_comp_pt_plot_converter( thetaEl_lb);
        thetaEl_ub_plot = genie_clas_el_mom_comp_pt_plot_converter( thetaEl_ub);
    }
    ...
    if (deltaPhiProtEq_kwd_pos != std::string::npos){
        ...
        deltaPhiProt_plot = genie_clas_el_mom_comp_pt_plot_converter( deltaPhiProt);
    }
    ...
    if (thetaProt_kwd_pos != std::string::npos){
        ...
        thetaProt_lb_plot = genie_clas_el_mom_comp_pt_plot_converter( thetaProt_lb);
        thetaProt_ub_plot = genie_clas_el_mom_comp_pt_plot_converter( thetaProt_ub);
    }
    ...
    //std::cout << "protMom_ub: " << protMom_ub << "; protMom_ub_plot: " << protMom_ub_plot << std::endl;
        ...
        protMom_lb_plot = genie_clas_el_mom_comp_pt_plot_converter( protMom_lb);
        protMom_ub_plot = genie_clas_el_mom_comp_pt_plot_converter( protMom_ub);
    }
    ...
    TH1F* mc_h1_el_momentum[6][4];
    TH1F* data_h1_el_momentum[6];
	...
	for (int j = 0; j < 4; j++) { // for all the interactions
	    ...
	    mc_h1_el_momentum[i][j] = (TH1F*)mc_input->Get(TString::Format("h1_%i_Omega_FullyInclusive_NoQ4Weight_Theta_Slice_InSector_el_mom__%i", j+1, i));
	    ...
	    mc_h1_el_momentum[i][0]->Add( mc_h1_el_momentum[i][j]);
	}
    ...
    for (int i = 0; i < 6; i++) { // for all the sectors
	data_h1_el_momentum[i] = (TH1F*)data_input->Get( TString::Format("h1_0_Omega_FullyInclusive_NoQ4Weight_Theta_Slice_InSector_el_mom__%i", i));
    }

    // compile all the sectors into one histogram
    for( int i = 1; i < 6; i++) { 
	mc_h1_el_momentum[0][0]->Add( mc_h1_el_momentum[i][0]);
	data_h1_el_momentum[0]->Add( data_h1_el_momentum[i]); 
    }
    ...
    mc_h1_el_momentum[0][0]->Sumw2();
    mc_h1_el_momentum[0][0]->Rebin( rebin_factor);
    mc_h1_el_momentum[0][0]->Scale( data_h1_el_momentum[0]->Integral()/mc_h1_el_momentum[0][0]->Integral()); // normalize MC to CLAS data
    mc_h1_el_momentum[0][0]->SetStats( 0); // get rid of the stats box that usually appears at the top right of plots
    mc_h1_el_momentum[0][0]->GetXaxis()->SetTitle("Electron Momentum [GeV/c]");
    mc_h1_el_momentum[0][0]->GetYaxis()->SetTitle("Events");
    mc_h1_el_momentum[0][0]->GetXaxis()->SetRangeUser( genie_clas_el_mom_comp_pt_converter( elMom_lb), 2.2);
    mc_h1_el_momentum[0][0]->GetXaxis()->CenterTitle( true);
    mc_h1_el_momentum[0][0]->GetYaxis()->CenterTitle( true);
    Double_t mc_y_max = mc_h1_el_momentum[0][0]->GetMaximum(); // get y-axis max for plotting purposes later

    data_h1_el_momentum[0]->Sumw2();
    data_h1_el_momentum[0]->Rebin( rebin_factor);
    data_h1_el_momentum[0]->SetStats( 0); // get rid of the stats box that usually appears at the top right of plots
    data_h1_el_momentum[0]->GetXaxis()->SetTitle("Electron Momentum [GeV/c]");
    data_h1_el_momentum[0]->GetYaxis()->SetTitle("Events");
    data_h1_el_momentum[0]->GetXaxis()->SetRangeUser( genie_clas_el_mom_comp_pt_converter( elMom_lb), 2.2);  
    data_h1_el_momentum[0]->GetXaxis()->CenterTitle( true);
    data_h1_el_momentum[0]->GetYaxis()->CenterTitle( true);
    data_h1_el_momentum[0]->SetMarkerStyle( kFullCircle);
    data_h1_el_momentum[0]->SetMarkerColor( kBlack);
    data_h1_el_momentum[0]->SetLineColor( kBlack);
    Double_t data_y_max = data_h1_el_momentum[0]->GetMaximum(); // get y-axis max for plotting purposes directly below

    Double_t mc_data_y_max = std::max( mc_y_max, data_y_max);
    mc_h1_el_momentum[0][0]->GetYaxis()->SetRangeUser( 0, 1.1*mc_data_y_max); // bound plot so we can see all the mc with some headroom
    data_h1_el_momentum[0]->GetYaxis()->SetRangeUser( 0, 1.1*mc_data_y_max); // bound plot so we can see all the data with some headroom

    gStyle->SetErrorX(0); // don't plot x error bars for any of the distributions

    if (cut_case.compare("noCuts") == 0) { 
	mc_h1_el_momentum[0][0]->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam (No Cuts)"); 
        data_h1_el_momentum[0]->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam (No Cuts)"); 
    }
    else if ( cut_case.compare("incl_noElMomCut") == 0) { 
	mc_h1_el_momentum[0][0]->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ}"); 
        data_h1_el_momentum[0]->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ}"); 
    }
    else if ( cut_case.compare("incl") == 0) { 
	mc_h1_el_momentum[0][0]->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+"}"); 
        data_h1_el_momentum[0]->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+"}");
    }
	...
	mc_h1_el_momentum[0][0]->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ}");
        data_h1_el_momentum[0]->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ}");
    }
    ...
    mc_h1_el_momentum[0][0]->Draw("HIST");
    data_h1_el_momentum[0]->Draw("E1 SAME");

    // draw a legend for our plot
    TLegend *legend = new TLegend( 0.2, 0.725, 0.35, 0.825);
    legend->AddEntry( mc_h1_el_momentum[0][0], "GENIE MC");
    legend->AddEntry( data_h1_el_momentum[0], "CLAS data");
    ...
    int num_events_mc = mc_h1_el_momentum[0][0]->Integral();
    int num_events_data = data_h1_el_momentum[0]->Integral();
    int num_events = num_events_mc + num_events_data;

    cout << "// --- PROGRAM OUTPUT -----------------------------------------------------------" << endl;
    cout << "There are " << num_events_mc << " many events in the GENIE MC h1_el_momentum histogram." << endl;
    cout << "There are " << num_events_data << " many events in the CLAS data h1_el_momentum histogram." << endl;
    cout << "There are " << num_events << " total events in both of the GENIE and CLAS h1_el_momentum histograms." << endl;
