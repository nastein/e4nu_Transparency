// The purpose of this script is to extract the h2_prot_mom_el_mom
...
int elMom_comp (const void * elem1, const void * elem2)
{
...
void prot_bin_study_elMom( std::string file_name){
   ...
   // Give a little preamble so the user can choose the bins they want to use.
    ...
    qsort( bin_bounds, sizeof(bin_bounds)/sizeof(*bin_bounds), sizeof(*bin_bounds), elMom_comp);
    ...
    // Form (rebin_factor_x, rebin_factor_y): (10, 10))
    Int_t rebin_factor_x = 10;
    ...
    double elMom_lb = ((TParameter<double>*)Run_Info->FindObject("El mom lb"))->GetVal();
    std::string run_type = std::string(((TNamed*)Run_Info->FindObject("Run type"))->GetTitle());

    // histogram initialization
    TH2F* h2_prot_mom_el_mom = (TH2F*)input1->Get(TString::Format("h2_prot_mom_el_mom"));
    ...
    h2_prot_mom_el_mom->RebinX( rebin_factor_x); // speeds up code by getting rid of statistics we won't necessarily look at
    h2_prot_mom_el_mom->RebinY( rebin_factor_y); // speeds up code by getting rid of statistics we won't necessarily look at

    TH2F* bin_selected_h2_prot_mom_el_mom[N_BINS];
    TH1D* bin_selected_h1_el_mom_proj[N_BINS];
    Int_t n_ybins;
    Int_t bin_for_lb;
    Int_t bin_for_ub;
    for (int i = 0; i < N_BINS; i++) {
        bin_selected_h2_prot_mom_el_mom[i] = (TH2F*)h2_prot_mom_el_mom->Clone(); // histogram will have proton momentum bin cuts applied
        
	n_ybins = bin_selected_h2_prot_mom_el_mom[i]->GetNbinsY();
	...
	bin_for_ub = (Int_t)( (bin_bounds[i+1] / 6) * n_ybins ); // factor of 6 because proton momentum ranges from 0 to 6
        ...
        bin_selected_h1_el_mom_proj[i] = (TH1D*)h2_prot_mom_el_mom->ProjectionX( TString::Format("h2_prot_mom_el_mom_bin%i_px", i+1), bin_for_lb, bin_for_ub)->Clone(); // project TH2 onto x-axis, only take values in given proton momentum band
    }
    
    // format the histograms
    h2_prot_mom_el_mom->SetStats( 0);
    h2_prot_mom_el_mom->GetXaxis()->SetRangeUser( 0.5, 2.2);
    h2_prot_mom_el_mom->GetYaxis()->SetRangeUser( protMom_lb, bin_bounds[N_BINS]);
    h2_prot_mom_el_mom->GetZaxis()->SetRangeUser( 1, h2_prot_mom_el_mom->GetMaximum()); // do not assign a color to bins with <= 0 events in them
    h2_prot_mom_el_mom->GetXaxis()->SetTitle("Electron Momentum [GeV]");
    h2_prot_mom_el_mom->GetYaxis()->SetTitle("Proton Momentum [GeV]");
    h2_prot_mom_el_mom->GetXaxis()->CenterTitle( true);
    h2_prot_mom_el_mom->GetYaxis()->CenterTitle( true);
    h2_prot_mom_el_mom->SetTitle(TString(info)+TString(cuts));
    ...
    for (int i = 0; i < N_BINS; i++) {
    	if (N_BINS != 1) { // if N_BINS == 1, this plot will be the same as TH2 histogram above bit with a different file name; we want to avoid that
        	    bin_selected_h2_prot_mom_el_mom[i]->SetStats( 0);
        	    bin_selected_h2_prot_mom_el_mom[i]->GetXaxis()->SetRangeUser( 0.5, 2.2);
        	    bin_selected_h2_prot_mom_el_mom[i]->GetYaxis()->SetRangeUser( bin_bounds[i], bin_bounds[i+1]);
    	    bin_selected_h2_prot_mom_el_mom[i]->GetZaxis()->SetRangeUser( 1, bin_selected_h2_prot_mom_el_mom[i]->GetMaximum()); // do not assign a color to bins with <=0 events in them
        	    bin_selected_h2_prot_mom_el_mom[i]->GetXaxis()->SetTitle("Electron Momentum [GeV]");
        	    bin_selected_h2_prot_mom_el_mom[i]->GetYaxis()->SetTitle("Proton Momentum [GeV]");
        	    bin_selected_h2_prot_mom_el_mom[i]->GetXaxis()->CenterTitle( true);
        	    bin_selected_h2_prot_mom_el_mom[i]->GetYaxis()->CenterTitle( true);
        	    bin_selected_h2_prot_mom_el_mom[i]->SetTitle("Proton Momentum Isolated "+TString(info)+TString(cuts)+" ("+TString( std::to_string( bin_bounds[i]))+" GeV < p_{p^{+}} < "+TString(  std::to_string( bin_bounds[i+1]))+" GeV)");
    	}

        bin_selected_h1_el_mom_proj[i]->SetStats( 0);
        bin_selected_h1_el_mom_proj[i]->Sumw2();
    	bin_selected_h1_el_mom_proj[i]->GetXaxis()->SetRangeUser( 0.5, 2.2);
    	bin_selected_h1_el_mom_proj[i]->GetXaxis()->SetTitle("Electron Momentum [GeV]");
   	    bin_selected_h1_el_mom_proj[i]->GetYaxis()->SetTitle("Events");
    	bin_selected_h1_el_mom_proj[i]->GetXaxis()->CenterTitle( true);
    	bin_selected_h1_el_mom_proj[i]->GetYaxis()->CenterTitle( true);
        if (run_type.find("Data") !=  string::npos) { 
	       bin_selected_h1_el_mom_proj[i]->SetMarkerColor( kBlack);
	       bin_selected_h1_el_mom_proj[i]->SetMarkerStyle( kFullCircle);
	       bin_selected_h1_el_mom_proj[i]->SetLineColor( kBlack);
	   }
	bin_selected_h1_el_mom_proj[i]->SetTitle(TString(info)+TString(cuts)+" ("+TString(std::to_string( bin_bounds[i]))+" GeV < p_{p^{+}} < "+TString(  std::to_string( bin_bounds[i+1]))+" GeV)");
    }
    gStyle->SetErrorX( 0); // do not plot x-axis errors
    ...
    if (run_type.find("Genie") !=  string::npos){ // include in file name that we have set the Mott cross section equal to 1 with the flag "MottXSecEq1"
	save_file_path = "../output/prot_bin_study_elMom/"+TString(info)+TString(cuts)+"protmom_vs_elmom_MottXSecEq1.pdf"; 
    }
    else if (run_type.find("Data") !=  string::npos) { // we can't set data to have a Mott cross section equal to 1, so we remove that flag here
	save_file_path = "../output/prot_bin_study_elMom/"+TString(info)+TString(cuts)+"protmom_vs_elmom.pdf"; 
    }
    ...
    TLegend* lg2 = new TLegend( 0.1, 0.6, 0.48, 0.9 );
    ...
    if (run_type.find("Genie") !=  string::npos) {
        for (int i = 0; i < N_BINS; i++) {
	       //if (N_BINS != 1) { // if N_BINS == 1, then this plot will be the same as the TH2 saved above; we don't want duplicate plots
	        //bin_selected_h2_prot_mom_el_mom[i]->Draw("colz");
	        //save_file_path = "../output/prot_bin_study_elMom/"+TString(file_type)+"_h2_prot_mom_el_mom_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+elMom_dot_converter( bin_bounds[i])+"to"+elMom_dot_converter( bin_bounds[i+1])+"_MottXSecEq1.pdf";
	    	//c->SetLogy( 0); // set y scale to be linear
	    	//c->SaveAs( TString::Format("%s", save_file_path.c_str()));
	       //}

            lg2->AddEntry(bin_selected_h1_el_mom_proj[i], (std::to_string( bin_bounds[i])+" < p_{p} < "+std::to_string( bin_bounds[i+1])).c_str(), "l");

	        if(i == 0) bin_selected_h1_el_mom_proj[i]->Draw("PLCHIST");
            else bin_selected_h1_el_mom_proj[i]->Draw("PLC HIST SAME");
        }
        save_file_path = "../output/prot_bin_study_elMom/"+TString(info)+TString(cuts)+"prot_mom_bin_studies_protmom_vs_elmom_MottXSecEq1.pdf";
        lg2->Draw( "same" );
	    c->SetLogy();
	    c->SaveAs( TString::Format("%s", save_file_path.c_str()));
    }
    // this is the loop to name all of the proton band separated files for CLAS data
        ...
        for (int i = 0; i < N_BINS; i++) {
            if (N_BINS != 1) { // if N_BINS == 1, then this plot will be the same as the TH2 saved above; we don't want duplicate plots
		bin_selected_h2_prot_mom_el_mom[i]->Draw("colz");
            	save_file_path = "../output/prot_bin_study_elMom/"+TString(info)+TString(cuts)+"_bin"+std::to_string( bin_bounds[i])+"to"+std::to_string( bin_bounds[i+1])+".pdf";
     	    	c->SetLogy( 0); // set y scale to be linear
            	c->SaveAs( TString::Format("%s", save_file_path.c_str()));
	    }

            bin_selected_h1_el_mom_proj[i]->Draw("E1");
            save_file_path = "../output/prot_bin_study_elMom/"+TString(info)+TString(cuts)+"_bin"+std::to_string( bin_bounds[i])+"to"+std::to_string( bin_bounds[i+1])+".pdf";
	    c->SetLogy();
	    c->SaveAs( TString::Format("%s", save_file_path.c_str()));
        }
...
void prot_bin_study_elMom( std::string file_name, Double_t bin_bounds[], const Int_t N_BINS){
    // CAUTION: These rebin_factor variables need to be large enough so that this
    //          in the TH2 extracted from the ROOT file passed to this script.
    //          As a courtesey to others, I have documented value pairs for which 
    //          this program executes.
    ...
    else { std::cout << "You have entered an invalid/unknown target. Consider adding code to accomodate this target in prot_bin_study_elMom.C." << std::endl; }
    ...
    else { std::cout << "You have entered an invalid/unknown beam energy. Consider adding code to accomodate this beam energy in prot_bin_study_elMom.C." << std::endl; }
	...
	deltaPhiEl_plot = elMom_pt_plot_converter( deltaPhiEl);
    }
	...
	thetaEl_lb_plot = elMom_pt_plot_converter( thetaEl_lb);
        thetaEl_ub_plot = elMom_pt_plot_converter( thetaEl_ub);
        if (elMom_pt_converter( thetaEl_lb) == 20 && elMom_pt_converter( thetaEl_ub) == 25) { 
	    range = "range1"; 
	    range_plot = "Range 1";
	}
        if (elMom_pt_converter( thetaEl_lb) == 30 && elMom_pt_converter( thetaEl_ub) == 35) { 
            range = "range2";  
            range_plot = "Range 2";
        }
        if (elMom_pt_converter( thetaEl_lb) == 40 && elMom_pt_converter( thetaEl_ub) == 45) { 
            range = "range3";  
            range_plot = "Range 3";
        }
    }
    ...
    if (elMom_kwd_pos != std::string::npos){
        ...
        elMom_lb_plot = elMom_pt_plot_converter( elMom_lb) + " GeV";
    }
    ...
    if (deltaPhiProtEq_kwd_pos != std::string::npos){
        ...
        deltaPhiProt_plot = elMom_pt_plot_converter( deltaPhiProt);
    }
    ...
    if (thetaProt_kwd_pos != std::string::npos){
        ...
        thetaProt_lb_plot = elMom_pt_plot_converter( thetaProt_lb);
        thetaProt_ub_plot = elMom_pt_plot_converter( thetaProt_ub);
    }
    ...
    //std::cout << "protMom_ub: " << protMom_ub << "; protMom_ub_plot: " << protMom_ub_plot << std::endl;
        ...
        protMom_lb_plot = elMom_pt_plot_converter( protMom_lb);
        protMom_ub_plot = elMom_pt_plot_converter( protMom_ub);
    }
    ...
    // format the histograms
    h2_prot_mom_el_mom->SetStats( 0);
    h2_prot_mom_el_mom->GetXaxis()->SetRangeUser( elMom_pt_converter( elMom_lb), 2.2);
    h2_prot_mom_el_mom->GetYaxis()->SetRangeUser( elMom_pt_converter( protMom_lb), bin_bounds[N_BINS]);
    h2_prot_mom_el_mom->GetZaxis()->SetRangeUser( 1, h2_prot_mom_el_mom->GetMaximum()); // do not assign a color to bins with <= 0 events in them
    h2_prot_mom_el_mom->GetXaxis()->SetTitle("Electron Momentum [GeV]");
    h2_prot_mom_el_mom->GetYaxis()->SetTitle("Proton Momentum [GeV]");
    h2_prot_mom_el_mom->GetXaxis()->CenterTitle( true);
    ...
    for (int i = 0; i < N_BINS; i++) {
        if (N_BINS != 1) { // if N_BINS == 1, this plot will be the same as TH2 histogram above bit with a different file name; we want to avoid that
            bin_selected_h2_prot_mom_el_mom[i]->SetStats( 0);
            bin_selected_h2_prot_mom_el_mom[i]->GetXaxis()->SetRangeUser( elMom_pt_converter( elMom_lb), 2.2);
            bin_selected_h2_prot_mom_el_mom[i]->GetYaxis()->SetRangeUser( bin_bounds[i], bin_bounds[i+1]);
            bin_selected_h2_prot_mom_el_mom[i]->GetZaxis()->SetRangeUser( 1, bin_selected_h2_prot_mom_el_mom[i]->GetMaximum()); // do not assign a color to bins with <=0 events in them
            bin_selected_h2_prot_mom_el_mom[i]->GetXaxis()->SetTitle("Electron Momentum [GeV]");
            bin_selected_h2_prot_mom_el_mom[i]->GetYaxis()->SetTitle("Proton Momentum [GeV]");
            bin_selected_h2_prot_mom_el_mom[i]->GetXaxis()->CenterTitle( true);
            bin_selected_h2_prot_mom_el_mom[i]->GetYaxis()->CenterTitle( true);
            bin_selected_h2_prot_mom_el_mom[i]->SetTitle("Proton Momentum Isolated "+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam, "+TString(file_type_plot)+" "+TString(range_plot)+" ("+TString( elMom_pt_plot_converter( std::to_string( bin_bounds[i])))+" GeV < p_{p^{+}} < "+TString( elMom_pt_plot_converter( std::to_string( bin_bounds[i+1])))+" GeV)");
        }

        bin_selected_h1_el_mom_proj[i]->SetStats( 0);
        bin_selected_h1_el_mom_proj[i]->Sumw2();
        bin_selected_h1_el_mom_proj[i]->GetXaxis()->SetRangeUser( elMom_pt_converter( elMom_lb), 2.2);
        bin_selected_h1_el_mom_proj[i]->GetXaxis()->SetTitle("Electron Momentum [GeV]");
        bin_selected_h1_el_mom_proj[i]->GetYaxis()->SetTitle("Events");
        bin_selected_h1_el_mom_proj[i]->GetXaxis()->CenterTitle( true);
        bin_selected_h1_el_mom_proj[i]->GetYaxis()->CenterTitle( true);
	...
	bin_selected_h1_el_mom_proj[i]->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam, "+TString(file_type_plot)+" "+TString(range_plot)+" ("+TString( elMom_pt_plot_converter( std::to_string( bin_bounds[i])))+" GeV < p_{p^{+}} < "+TString( elMom_pt_plot_converter( std::to_string( bin_bounds[i+1])))+" GeV)");
    }
    gStyle->SetErrorX( 0);
    ...
    if (file_type.compare("genie") == 0){ // include in file name that we have set the Mott cross section equal to 1 with the flag "MottXSecEq1"
        save_file_path = "../output/prot_bin_study_elMom/"+TString(file_type)+"_h2_prot_mom_el_mom_target"+TString(target)+"_beamEnergy"+TString(beam_energy)+"_MottXSecEq1_"+TString(range)+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMom"+TString(protMom_lb)+"to"+TString(protMom_ub)+".pdf";
    }
    else if (file_type.compare("data") == 0) { // we can't set data to have a Mott cross section equal to 1, so we remove that flag here
        save_file_path = "../output/prot_bin_study_elMom/"+TString(file_type)+"_h2_prot_mom_el_mom_target"+TString(target)+"_beamEnergy"+TString(beam_energy)+"_"+TString(range)+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMom"+TString(protMom_lb)+"to"+TString(protMom_ub)+".pdf";
    }
    c->SaveAs( TString::Format("%s", save_file_path.c_str()));
    ...
    if (file_type.compare("genie") == 0) {
        for (int i = 0; i < N_BINS; i++) {
            ...
            bin_selected_h1_el_mom_proj[i]->Draw("HIST");
            save_file_path = "../output/prot_bin_study_elMom/"+TString(file_type)+"_h1_el_mom_proj_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+elMom_dot_converter( bin_bounds[i])+"to"+elMom_dot_converter( bin_bounds[i+1])+"_MottXSecEq1.pdf";
            c->SetLogy();
            c->SaveAs( TString::Format("%s", save_file_path.c_str()));
        }
    }
    // this is the loop to name all of the proton band separated files for CLAS data
        ...
        for (int i = 0; i < N_BINS; i++) {
            if (N_BINS != 1) { // if N_BINS == 1, then this plot will be the same as the TH2 saved above; we don't want duplicate plots
                bin_selected_h2_prot_mom_el_mom[i]->Draw("colz");
                save_file_path = "../output/prot_bin_study_elMom/"+TString(file_type)+"_h2_prot_mom_el_mom_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+elMom_dot_converter( bin_bounds[i])+"to"+elMom_dot_converter( bin_bounds[i+1])+".pdf";
                c->SetLogy( 0); // set y scale to be linear
                c->SaveAs( TString::Format("%s", save_file_path.c_str()));
            }

            bin_selected_h1_el_mom_proj[i]->Draw("E1");
            save_file_path = "../output/prot_bin_study_elMom/"+TString(file_type)+"_h1_el_mom_proj_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+elMom_dot_converter( bin_bounds[i])+"to"+elMom_dot_converter( bin_bounds[i+1])+".pdf";
            c->SetLogy();
            c->SaveAs( TString::Format("%s", save_file_path.c_str()));
        }
    }
