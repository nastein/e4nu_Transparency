// The purpose of this script is to extract the h2_prot_mom_theta
...
double protTheta_pt_converter( std::string str) {
...
std::string protTheta_dot_converter( double val) {
...
// -----------------------------------------------------------------------------------------------------------------------
// This auxillary function utilizes protTheta_pt_converter to take the resulting
// value from protTheta_pt_converter, remove any trailing zeros, and then recast
// the double from protTheta_pt_converter to be a std::string.
std::string protTheta_pt_plot_converter( std::string str) {
    str = std::to_string( protTheta_pt_converter( str)); // replace 'pt' -> '.' and recast as a std::string
    while (str.substr( str.length() - 1).compare("0") == 0){ // while the last character in str is a 0
        str = str.substr( 0, str.length() - 1); // take off the trailing 0
    }
...
int protTheta_comp (const void * elem1, const void * elem2)
{
...
void prot_bin_study_protTheta( std::string file_name){
   ...
   // Give a little preamble so the user can choose the bins they want to use.
    ...
    qsort( bin_bounds, sizeof(bin_bounds)/sizeof(*bin_bounds), sizeof(*bin_bounds), protTheta_comp);
    ...
    else { std::cout << "You have entered an invalid/unknown target. Consider adding code to accomodate this target in prot_bin_study_protTheta.C." << std::endl; }
    ...
    else { std::cout << "You have entered an invalid/unknown beam energy. Consider adding code to accomodate this beam energy in prot_bin_study_protTheta.C." << std::endl; }
	...
	deltaPhiEl_plot = protTheta_pt_plot_converter( deltaPhiEl);
    }
	...
	thetaEl_lb_plot = protTheta_pt_plot_converter( thetaEl_lb);
        thetaEl_ub_plot = protTheta_pt_plot_converter( thetaEl_ub);
        if (protTheta_pt_converter( thetaEl_lb) == 20 && protTheta_pt_converter( thetaEl_ub) == 25) { 
	    range = "range1"; 
	    range_plot = "Range 1";
	}
        if (protTheta_pt_converter( thetaEl_lb) == 30 && protTheta_pt_converter( thetaEl_ub) == 35) { 
            range = "range2";  
            range_plot = "Range 2";
        }
        if (protTheta_pt_converter( thetaEl_lb) == 40 && protTheta_pt_converter( thetaEl_ub) == 45) { 
            range = "range3";  
            range_plot = "Range 3";
        }
    }
    ...
    if (elMom_kwd_pos != std::string::npos){
        ...
        elMom_lb_plot = protTheta_pt_plot_converter( elMom_lb) + " GeV";
    }
    ...
    if (deltaPhiProtEq_kwd_pos != std::string::npos){
        ...
        deltaPhiProt_plot = protTheta_pt_plot_converter( deltaPhiProt);
    }
    ...
    if (thetaProt_kwd_pos != std::string::npos){
        ...
        thetaProt_lb_plot = protTheta_pt_plot_converter( thetaProt_lb);
        thetaProt_ub_plot = protTheta_pt_plot_converter( thetaProt_ub);
    }
    ...
    //std::cout << "protMom_ub: " << protMom_ub << "; protMom_ub_plot: " << protMom_ub_plot << std::endl;
        ...
        protMom_lb_plot = protTheta_pt_plot_converter( protMom_lb);
        protMom_ub_plot = protTheta_pt_plot_converter( protMom_ub);
    }
    ...
    TH2F* h2_prot_mom_theta = (TH2F*)input1->Get(TString::Format("h2_prot_mom_theta"));
    ...
    h2_prot_mom_theta->RebinX( rebin_factor_x); // speeds up code by getting rid of statistics we won't necessarily look at
    h2_prot_mom_theta->RebinY( rebin_factor_y); // speeds up code by getting rid of statistics we won't necessarily look at

    TH2F* bin_selected_h2_prot_mom_theta[N_BINS];
    TH1D* bin_selected_h1_prot_theta_proj[N_BINS];
    Int_t n_ybins;
    Int_t bin_for_lb;
    Int_t bin_for_ub;
    for (int i = 0; i < N_BINS; i++) {
        bin_selected_h2_prot_mom_theta[i] = (TH2F*)h2_prot_mom_theta->Clone(); // histogram will have proton momentum bin cuts applied
        
	n_ybins = bin_selected_h2_prot_mom_theta[i]->GetNbinsY();
	...
	bin_for_ub = (Int_t)( (bin_bounds[i+1] / 6) * n_ybins ); // factor of 6 because proton momentum ranges from 0 to 6
        ...
        bin_selected_h1_prot_theta_proj[i] = (TH1D*)h2_prot_mom_theta->ProjectionX( TString::Format("h2_prot_mom_theta_bin%i_px", i+1), bin_for_lb, bin_for_ub)->Clone(); // project TH2 onto x-axis, only take values in given proton momentum band
    }
    
    // format the histograms
    h2_prot_mom_theta->SetStats( 0);
    h2_prot_mom_theta->GetXaxis()->SetRangeUser( protTheta_pt_converter( thetaProt_lb), protTheta_pt_converter( thetaProt_ub));
    h2_prot_mom_theta->GetYaxis()->SetRangeUser( protTheta_pt_converter( protMom_lb), bin_bounds[N_BINS]);
    h2_prot_mom_theta->GetZaxis()->SetRangeUser( 1, h2_prot_mom_theta->GetMaximum()); // do not assign a color to bins with <= 0 events in them
    h2_prot_mom_theta->GetXaxis()->SetTitle("#theta_{proton} [deg]");
    h2_prot_mom_theta->GetYaxis()->SetTitle("Proton Momentum [GeV]");
    h2_prot_mom_theta->GetXaxis()->CenterTitle( true);
    h2_prot_mom_theta->GetYaxis()->CenterTitle( true);
    if (protMom_lb_kwd_pos != std::string::npos) {
        h2_prot_mom_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ, p_{p^{+}} > "+TString(protMom_lb_plot)+"}");
    }
    else if (protMom_ub_kwd_pos != std::string::npos) {
	...
	if (N_BINS != 1) { // if N_BINS == 1, this plot will be the same as TH2 histogram above bit with a different file name; we want to avoid that
    	    bin_selected_h2_prot_mom_theta[i]->SetStats( 0);
    	    bin_selected_h2_prot_mom_theta[i]->GetXaxis()->SetRangeUser( protTheta_pt_converter( thetaProt_lb), protTheta_pt_converter( thetaProt_ub));
    	    bin_selected_h2_prot_mom_theta[i]->GetYaxis()->SetRangeUser( bin_bounds[i], bin_bounds[i+1]);
	    bin_selected_h2_prot_mom_theta[i]->GetZaxis()->SetRangeUser( 1, bin_selected_h2_prot_mom_theta[i]->GetMaximum()); // do not assign a color to bins with <=0 events in them
    	    bin_selected_h2_prot_mom_theta[i]->GetXaxis()->SetTitle("#theta_{proton} [deg]");
    	    bin_selected_h2_prot_mom_theta[i]->GetYaxis()->SetTitle("Proton Momentum [GeV]");
    	    bin_selected_h2_prot_mom_theta[i]->GetXaxis()->CenterTitle( true);
    	    bin_selected_h2_prot_mom_theta[i]->GetYaxis()->CenterTitle( true);
    	    bin_selected_h2_prot_mom_theta[i]->SetTitle("Proton Momentum Isolated "+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam, "+TString(file_type_plot)+" "+TString(range_plot)+" ("+TString( protTheta_pt_plot_converter( std::to_string( bin_bounds[i])))+" GeV < p_{p^{+}} < "+TString( protTheta_pt_plot_converter( std::to_string( bin_bounds[i+1])))+" GeV)");
	}

    	bin_selected_h1_prot_theta_proj[i]->SetStats( 0);
	bin_selected_h1_prot_theta_proj[i]->Sumw2();
    	bin_selected_h1_prot_theta_proj[i]->GetXaxis()->SetRangeUser( protTheta_pt_converter( thetaProt_lb), protTheta_pt_converter( thetaProt_ub));
    	bin_selected_h1_prot_theta_proj[i]->GetXaxis()->SetTitle("#theta_{proton} [deg]");
   	bin_selected_h1_prot_theta_proj[i]->GetYaxis()->SetTitle("Events");
    	bin_selected_h1_prot_theta_proj[i]->GetXaxis()->CenterTitle( true);
    	bin_selected_h1_prot_theta_proj[i]->GetYaxis()->CenterTitle( true);
        if (file_type.compare("data") == 0) {
            bin_selected_h1_prot_theta_proj[i]->SetMarkerColor( kBlack);
            bin_selected_h1_prot_theta_proj[i]->SetMarkerStyle( kFullCircle);
            bin_selected_h1_prot_theta_proj[i]->SetLineColor( kBlack);
        }
	bin_selected_h1_prot_theta_proj[i]->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam, "+TString(file_type_plot)+" "+TString(range_plot)+" ("+TString( protTheta_pt_plot_converter( std::to_string( bin_bounds[i])))+" GeV < p_{p^{+}} < "+TString( protTheta_pt_plot_converter( std::to_string( bin_bounds[i+1])))+" GeV)");
    }
    gStyle->SetErrorX( 0);
	...
	save_file_path = "../output/prot_bin_study_protTheta/"+TString(file_type)+"_h2_prot_mom_theta_target"+TString(target)+"_beamEnergy"+TString(beam_energy)+"_MottXSecEq1_"+TString(range)+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMom"+TString(protMom_lb)+"to"+TString(protMom_ub)+".pdf"; 
    }
    else if (file_type.compare("data") == 0) { // we can't set data to have a Mott cross section equal to 1, so we remove that flag here
	save_file_path = "../output/prot_bin_study_protTheta/"+TString(file_type)+"_h2_prot_mom_theta_target"+TString(target)+"_beamEnergy"+TString(beam_energy)+"_"+TString(range)+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMom"+TString(protMom_lb)+"to"+TString(protMom_ub)+".pdf"; 
    }
    ...
    if (file_type.compare("genie") == 0) {
	    ...
	    if (N_BINS != 1) { // if N_BINS == 1, then this plot will be the same as the TH2 saved above; we don't want duplicate plots
	        bin_selected_h2_prot_mom_theta[i]->Draw("colz");
	        save_file_path = "../output/prot_bin_study_protTheta/"+TString(file_type)+"_h2_prot_mom_theta_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+protTheta_dot_converter( bin_bounds[i])+"to"+protTheta_dot_converter( bin_bounds[i+1])+"_MottXSecEq1.pdf";
	    	c->SetLogy( 0); // set y scale to be linear
	    	c->SaveAs( TString::Format("%s", save_file_path.c_str()));
	    }

	    bin_selected_h1_prot_theta_proj[i]->Draw("HIST");
            save_file_path = "../output/prot_bin_study_protTheta/"+TString(file_type)+"_h1_prot_theta_proj_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+protTheta_dot_converter( bin_bounds[i])+"to"+protTheta_dot_converter( bin_bounds[i+1])+"_MottXSecEq1.pdf";
	    c->SetLogy();
	    c->SaveAs( TString::Format("%s", save_file_path.c_str()));
	}
    }
    // this is the loop to name all of the proton band separated files for CLAS data
		...
		bin_selected_h2_prot_mom_theta[i]->Draw("colz");
            ...
            bin_selected_h1_prot_theta_proj[i]->Draw("E1");
            save_file_path = "../output/prot_bin_study_protTheta/"+TString(file_type)+"_h1_prot_theta_proj_target"+target+"_beamEnergy"+beam_energy+"_"+range+"_elMomLBEq"+TString(elMom_lb)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_bin"+protTheta_dot_converter( bin_bounds[i])+"to"+protTheta_dot_converter( bin_bounds[i+1])+".pdf";
	    c->SetLogy();
	    c->SaveAs( TString::Format("%s", save_file_path.c_str()));
        }
...
void prot_bin_study_protTheta( std::string file_name, Double_t bin_bounds[], const Int_t N_BINS){
    // CAUTION: These rebin_factor variables need to be large enough so that this
    //          in the TH2 extracted from the ROOT file passed to this script.
    //          As a courtesey to others, I have documented value pairs for which 
    //          this program executes.
