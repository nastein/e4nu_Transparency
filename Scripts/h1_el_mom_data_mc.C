...
void h1_el_mom_data_mc(std::string sim, std::string targ, std::string beamen, std::string range, std::string type){
   ...
   gStyle->SetOptTitle(0);
    ...
    file_name1 = "/genie/app/users/nsteinbe/grahams_e4nu/CLAS/GENIE/Reconstructed/ThetaPQCut_nominal/SuSAv2/Excl_Range"+range+"_Genie_"+sim_num+"_"+targ+"_"+beamen+".root";
    ...
    //PrettyDoubleXSecPlot(h1_elmom_data[0], 1, beam_energy, 4, false);
    ...
    h1_elmom_data[0]->SetStats( 0); // get rid of the stats box that usually appears at the top right of plots
   ...
   if(targ == "56Fe") {
    ...
    c3->SaveAs(TString::Format("ElecMomcut/ElecMomcut_%s_%s_%s.pdf",targ.c_str(), beamen.c_str(), range.c_str()));
