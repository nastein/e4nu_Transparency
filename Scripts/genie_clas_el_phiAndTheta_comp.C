...
// h2_Electron_Theta_Phi histogram between GENIE Monte 
...
double genie_clas_el_phiAndTheta_comp_pt_converter( std::string str) {
...
// -----------------------------------------------------------------------------------------------------------------------
// This auxillary function utilizes genie_clas_el_phiAndTheta_comp_pt_converter to take the resulting
// value from genie_clas_el_phiAndTheta_comp_pt_converter, remove any trailing zeros, and then recast
// the double from genie_clas_el_phiAndTheta_comp_pt_converter to be a std::string.
std::string genie_clas_el_phiAndTheta_comp_pt_plot_converter( std::string str) {
    str = std::to_string( genie_clas_el_phiAndTheta_comp_pt_converter( str)); // replace 'pt' -> '.' and recast as a std::string
...
// MAIN FUNCTION ---------------------------------------------------------------------------------------------------------
void genie_clas_el_phiAndTheta_comp( std::string mc_file_name, std::string data_file_name){
    ...
    else { std::cout << "You have entered an invalid/unknown target. Consider adding code to accomodate this target in genie_clas_el_phiAndTheta_comp.C." << std::endl; }
    ...
    else { std::cout << "You have entered an invalid/unknown beam energy. Consider adding code to accomodate this beam energy in genie_clas_el_phiAndTheta_comp.C" << std::endl; }
    ...
    if (deltaPhiElEq_kwd_pos != std::string::npos){
        ...
        deltaPhiEl_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( deltaPhiEl);
    }
    ...
    if (thetaEl_kwd_pos != std::string::npos){
        ...
        thetaEl_lb_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( thetaEl_lb);
        thetaEl_ub_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( thetaEl_ub);
    }
    ...
    if (deltaPhiProtEq_kwd_pos != std::string::npos){
        ...
        deltaPhiProt_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( deltaPhiProt);
    }
    ...
    if (thetaProt_kwd_pos != std::string::npos){
        ...
        thetaProt_lb_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( thetaProt_lb);
        thetaProt_ub_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( thetaProt_ub);
    }
    ...
    //std::cout << "protMom_ub: " << protMom_ub << "; protMom_ub_plot: " << protMom_ub_plot << std::endl;
        ...
        protMom_lb_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( protMom_lb);
        protMom_ub_plot = genie_clas_el_phiAndTheta_comp_pt_plot_converter( protMom_ub);
    }
    ...
    TH2F* mc_h2_Electron_Theta_Phi = (TH2F*)mc_input->Get(TString::Format("h2_Electron_Theta_Phi"));
    TH2F* data_h2_Electron_Theta_Phi = (TH2F*)data_input->Get(TString::Format("h2_Electron_Theta_Phi"));
    
    TH1D* mc_h1_el_phi = (TH1D*)mc_h2_Electron_Theta_Phi->ProjectionX()->Clone();
    TH1D* mc_h1_el_theta = (TH1D*)mc_h2_Electron_Theta_Phi->ProjectionY()->Clone();
    TH1D* data_h1_el_phi = (TH1D*)data_h2_Electron_Theta_Phi->ProjectionX()->Clone();
    TH1D* data_h1_el_theta = (TH1D*)data_h2_Electron_Theta_Phi->ProjectionY()->Clone();

    // normalize the GENIE MC to the level of the CLAS data
    mc_h1_el_phi->Scale( data_h1_el_phi->Integral()/mc_h1_el_phi->Integral()); 
    mc_h1_el_theta->Scale( data_h1_el_theta->Integral()/mc_h1_el_theta->Integral());
    ...
    mc_h1_el_phi->Rebin( 4);
    mc_h1_el_phi->Sumw2();
    mc_h1_el_phi->SetStats( 0);
    mc_h1_el_phi->GetXaxis()->SetRangeUser( 0, 360);
    mc_h1_el_phi->GetXaxis()->SetTitle("#phi_{electron} [deg]");
    mc_h1_el_phi->GetYaxis()->SetTitle("Events");
    mc_h1_el_phi->GetXaxis()->CenterTitle( true);
    mc_h1_el_phi->GetYaxis()->CenterTitle( true);
    Double_t mc_el_phi_y_max = mc_h1_el_phi->GetMaximum(); // get y-axis max for plotting purposes later

    mc_h1_el_theta->Sumw2();
    mc_h1_el_theta->SetStats( 0);
    if (thetaEl_kwd_pos != std::string::npos) { mc_h1_el_theta->GetXaxis()->SetRangeUser( genie_clas_el_phiAndTheta_comp_pt_converter( thetaEl_lb), genie_clas_el_phiAndTheta_comp_pt_converter( thetaEl_ub)); }
    else { mc_h1_el_theta->GetXaxis()->SetRangeUser( 20, 60); }
    mc_h1_el_theta->GetXaxis()->SetTitle("#theta_{electron} [deg]");
    mc_h1_el_theta->GetYaxis()->SetTitle("Events");
    mc_h1_el_theta->GetXaxis()->CenterTitle( true);
    mc_h1_el_theta->GetYaxis()->CenterTitle( true);
    Double_t mc_el_theta_y_max = mc_h1_el_theta->GetMaximum(); // get y-axis max foor plotting purposes later

    data_h1_el_phi->Rebin( 4);
    data_h1_el_phi->Sumw2();
    data_h1_el_phi->SetStats( 0);
    data_h1_el_phi->GetXaxis()->SetRangeUser( 0, 360);
    data_h1_el_phi->GetXaxis()->SetTitle("#phi_{electron} [deg]");
    data_h1_el_phi->GetYaxis()->SetTitle("Events");
    data_h1_el_phi->GetXaxis()->CenterTitle( true);
    data_h1_el_phi->GetYaxis()->CenterTitle( true);
    data_h1_el_phi->SetMarkerStyle( kFullCircle);
    data_h1_el_phi->SetMarkerColor( kBlack);
    data_h1_el_phi->SetMarkerSize( 0.6);
    data_h1_el_phi->SetLineColor( kBlack);
    Double_t data_el_phi_y_max = data_h1_el_phi->GetMaximum(); // get y-axis max for plotting purposes later

    data_h1_el_theta->Sumw2();
    data_h1_el_theta->SetStats( 0);
    if (thetaEl_kwd_pos != std::string::npos) { data_h1_el_theta->GetXaxis()->SetRangeUser( genie_clas_el_phiAndTheta_comp_pt_converter( thetaEl_lb), genie_clas_el_phiAndTheta_comp_pt_converter( thetaEl_ub)); }
    else { data_h1_el_theta->GetXaxis()->SetRangeUser( 20, 60); }
    data_h1_el_theta->GetXaxis()->SetTitle("#theta_{electron} [deg]");
    data_h1_el_theta->GetYaxis()->SetTitle("Events");
    data_h1_el_theta->GetXaxis()->CenterTitle( true);
    data_h1_el_theta->GetYaxis()->CenterTitle( true);
    data_h1_el_theta->SetMarkerStyle( kFullCircle);
    data_h1_el_theta->SetMarkerColor( kBlack);
    data_h1_el_theta->SetLineColor( kBlack);
    Double_t data_el_theta_y_max = data_h1_el_theta->GetMaximum(); // get y-axis max for plotting purposes later

    Double_t phi_max = std::max( mc_el_phi_y_max, data_el_phi_y_max);
    Double_t theta_max = std::max( mc_el_theta_y_max, data_el_theta_y_max);

    mc_h1_el_phi->GetYaxis()->SetRangeUser( 0, 1.1*phi_max); // multiply by 1.1 to give the plot some headroom
    data_h1_el_phi->GetYaxis()->SetRangeUser( 0, 1.1*phi_max); // multiply by 1.1 to give the plot some headroom
    mc_h1_el_theta->GetYaxis()->SetRangeUser( 0, 1.1*theta_max); // multiply by 1.1 to give the plot some headroom
    data_h1_el_theta->GetYaxis()->SetRangeUser( 0, 1.1*theta_max); // multiply by 1.1 to give the plot some headroom

    gStyle->SetErrorX(0); // don't plot x error bars for any of the distributions


    if (cut_case.compare("noCuts") == 0) {
        mc_h1_el_theta->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam (No Cuts)");
        mc_h1_el_phi->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam (No Cuts)");
        data_h1_el_theta->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam (No Cuts)");
        data_h1_el_phi->SetTitle(TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam (No Cuts)");
    }

    else if ( cut_case.compare("incl_noElMomCut") == 0) {
        mc_h1_el_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ}");
        mc_h1_el_phi->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ}");
        data_h1_el_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ}");
        data_h1_el_phi->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ}");
    }

    else if ( cut_case.compare("incl") == 0) {
        mc_h1_el_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+"}");
        mc_h1_el_phi->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+"}");
        data_h1_el_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+"}");
        data_h1_el_phi->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+"}");
    }

    else if ( cut_case.compare("excl") == 0) {
        mc_h1_el_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ}");
        mc_h1_el_phi->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ}");
        data_h1_el_theta->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ}");
        data_h1_el_phi->SetTitle("#splitline{"+TString(target_plot)+" with "+TString(beam_energy_plot)+" Beam}{#Delta#phi_{e^{-}} = "+TString(deltaPhiEl_plot)+"#circ, e^{-} Sectors "+TString(elSectors_plot)+", "+TString(thetaEl_lb_plot)+"#circ < #theta_{e^{-}} < "+TString(thetaEl_ub_plot)+"#circ, p_{e^{-}} > "+TString(elMom_lb_plot)+", p^{+} Sectors "+TString(protSectors_plot)+", #Delta#phi_{p^{+}} = "+TString(deltaPhiProt_plot)+", "+TString(thetaProt_lb_plot)+"#circ < #theta_{p^{+}} < "+TString(thetaProt_ub_plot)+"#circ}");
    }
    ...
    if ( cut_case.compare("noCuts") == 0) { 
        h1_el_theta_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_theta_target"+mc_target+"_beamEnergy"+mc_beam_energy+"_MottXSecEq1_noCuts.pdf"; 
        h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+mc_target+"_beamEnergy"+mc_beam_energy+"_MottXSecEq1_noCuts.pdf"; 
    }
    
    else if ( cut_case.compare("incl_noElMomCut") == 0) { 
        h1_el_theta_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_theta_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+".pdf";
        h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+".pdf";
    }

    else if ( cut_case.compare("incl") == 0) { 
        h1_el_theta_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_theta_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+"_elMomLBEq"+TString(elMom_lb)+".pdf"; 
        h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+"_elMomLBEq"+TString(elMom_lb)+".pdf"; 
    }
	...
	h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+"_elMomLBEq"+TString(elMom_lb)+"_protSectors"+TString(protSectors)+"_deltaPhiProtEq"+TString(deltaPhiProt)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+".pdf";
}
	...
	h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+"_elMomLBEq"+TString(elMom_lb)+"_protSectors"+TString(protSectors)+"_deltaPhiProtEq"+TString(deltaPhiProt)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMomLBEq"+TString(protMom_lb)+".pdf"; 
}
        ...
        h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+"_elMomLBEq"+TString(elMom_lb)+"_protSectors"+TString(protSectors)+"_deltaPhiProtEq"+TString(deltaPhiProt)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMomUBEq"+TString(protMom_ub)+".pdf";
}
        ...
        h1_el_phi_save_path = "../output/genie_clas_el_phiAndTheta_comp/genie_clas_comparison_h1_el_phi_target"+TString(mc_target)+"_beamEnergy"+TString(mc_beam_energy)+"_MottXSecEq1_elSectors"+TString(elSectors)+"_deltaPhiEq"+TString(deltaPhiEl)+"_thetaEl"+TString(thetaEl_lb)+"to"+TString(thetaEl_ub)+"_elMomLBEq"+TString(elMom_lb)+"_protSectors"+TString(protSectors)+"_deltaPhiProtEq"+TString(deltaPhiProt)+"_thetaProt"+TString(thetaProt_lb)+"to"+TString(thetaProt_ub)+"_protMom"+TString(protMom_lb)+"to"+TString(protMom_ub)+".pdf";
}


    // draw the phi histogram and save it as a PDF file
    mc_h1_el_phi->Draw("HIST");
    data_h1_el_phi->Draw("E1 SAME");

    TLegend *phi_legend = new TLegend( 0.475, 0.725, 0.725, 0.825);
    phi_legend->AddEntry( mc_h1_el_phi, "GENIE MC");
    phi_legend->AddEntry( data_h1_el_phi, "CLAS data");
    ...
    mc_h1_el_theta->Draw("HIST");
    data_h1_el_theta->Draw("E1 SAME");
    
    // draw a legent for out theta plot
    TLegend *theta_legend = new TLegend( 0.7, 0.725, 0.85, 0.825);
    theta_legend->AddEntry( mc_h1_el_theta, "GENIE MC");
    theta_legend->AddEntry( data_h1_el_theta, "CLAS data");
    ...
    //          it to the screen
    int mc_num_events = mc_h2_Electron_Theta_Phi->Integral();
    int data_num_events = data_h2_Electron_Theta_Phi->Integral();
    int num_events = mc_num_events + data_num_events;
    cout << "// --- PROGRAM OUTPUT ---------------------------------------------------------------------------" << endl;
    cout << "There are " << mc_num_events << " many events in the mc_h2_Electron_Theta_Phi histogram." << endl;
    cout << "There are " << data_num_events << " many events in the data_h2_Electron_Theta_Phi histogram." << endl;
    cout << "There are " << num_events << " many total events in both of the MC and data h2_Electron_Theta_Phi histograms." << endl;
