...
void prot_trans_vs_prot_mom_targetC12_beamEnergy1161_exclusivePlus(){
...
//	double corrRange[3] = {0.73469447,0.67918725,0.66175208};
	//double corrRange[3] = {0.75370356, 0.72307660, 0.70541045};
...
// Users don't have to read all the code below this dashed line unless they want to do a deep dive into the code, or if they want to debug, etc.
	...
	//std::string mc_file_path ("/genie/app/users/gchamber/e4nu_2022/e4nu/output/output2022/Analysis14/test2/");
	std::string mc_file_path ("/genie/app/users/gchamber/e4nu_2022/e4nu/output/output2022/Analysis22/");
	...
	std::string mc_incl[N_RANGES];
        ...
        mc_incl[0] = mc_file_path + "Inclusive/Inclusive_Range1_Genie_2_C12_2.261000.root";
        mc_incl[1] = mc_file_path + "Inclusive/Inclusive_Range2_Genie_2_C12_2.261000.root";
        mc_incl[2] = mc_file_path + "Inclusive/Inclusive_Range3_Genie_2_C12_2.261000.root";
        mc_excl[0] = mc_file_path + "Exclusive/Exclusive_Range1_Genie_2_C12_2.261000.root";
        mc_excl[1] = mc_file_path + "Exclusive/Exclusive_Range2_Genie_2_C12_2.261000.root";
        mc_excl[2] = mc_file_path + "Exclusive/Exclusive_Range3_Genie_2_C12_2.261000.root";

        data_incl[0] = data_file_path + "Inclusive/Inclusive_Range1_Data__C12_2.261000.root";
        data_incl[1] = data_file_path + "Inclusive/Inclusive_Range2_Data__C12_2.261000.root";
        data_incl[2] = data_file_path + "Inclusive/Inclusive_Range3_Data__C12_2.261000.root";
        data_excl[0] = data_file_path + "Exclusive/Exclusive_Range1_Data__C12_2.261000.root";
        data_excl[1] = data_file_path + "Exclusive/Exclusive_Range2_Data__C12_2.261000.root";
        data_excl[2] = data_file_path + "Exclusive/Exclusive_Range3_Data__C12_2.261000.root";
	...
	double qel[N_RANGES] = {0,0,0};
	double qel_err[N_RANGES] = {0,0,0};
	double mec[N_RANGES] = {0,0,0};
	double mec_err[N_RANGES] = {0,0,0};
	double res[N_RANGES] = {0,0,0};
	double res_err[N_RANGES] = {0,0,0};
	double dis[N_RANGES] = {0,0,0};
	double dis_err[N_RANGES] = {0,0,0};
	...
	for (int file_i = 0; file_i < N_RANGES; file_i++){ 
		...
		for (int sector = 0; sector < N_SECTORS; sector++) {
			...
			for (int interaction = 0; interaction < N_INT; interaction++) {
				...
				if (interaction == 0) {
					qel[file_i] = qel[file_i] + h1_el_mom_sectors_interactions_mc[file_i][sector][interaction]->IntegralAndError(1,h1_el_mom_sectors_interactions_mc[file_i][sector][interaction]->GetNbinsX(),qel_err[file_i]);}
				if (interaction == 1) {
					mec[file_i] = mec[file_i] + h1_el_mom_sectors_interactions_mc[file_i][sector][interaction]->IntegralAndError(1,h1_el_mom_sectors_interactions_mc[file_i][sector][interaction]->GetNbinsX(),mec_err[file_i]);}
				if (interaction == 2) {res[file_i] = res[file_i] + h1_el_mom_sectors_interactions_mc[file_i][sector][interaction]->Integral();}
				if (interaction == 3) {dis[file_i] = dis[file_i] + h1_el_mom_sectors_interactions_mc[file_i][sector][interaction]->Integral();}
	...
	std::cout << "qel, range " << file_i << ": " << qel[file_i] << " +/- " << qel_err[file_i] << std::endl;
	std::cout << "mec, range " << file_i << ": " << mec[file_i] << " +/- " << mec_err[file_i] << std::endl;
	//std::cout << "res, range " << file_i << " " << res[file_i] << std::endl;
	//std::cout << "dis, range " << file_i << " " << dis[file_i] << std::endl;
	double error1 = pow(pow(qel_err[file_i], 2) + pow(mec_err[file_i], 2), 0.5);
	double error2 = mec[file_i]/(qel[file_i]+mec[file_i]) * pow(pow(mec_err[file_i]/mec[file_i], 2) + pow(error1/(qel[file_i]+mec[file_i]), 2), 0.5);
	//std::cout << "mec+qel error: " << error1 << std::endl;
		
	std::cout << "mec/(qel+mec), range " << file_i << ": " << mec[file_i]/(qel[file_i]+mec[file_i]) << " +/- " << error2 << std::endl;
